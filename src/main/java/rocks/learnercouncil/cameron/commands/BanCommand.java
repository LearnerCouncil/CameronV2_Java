...
public class BanCommand extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        ...
        if(!Objects.requireNonNull(event.getMember()).hasPermission(Permission.BAN_MEMBERS))
            ...
            event.reply("Are you sure you want to ban " + user.getAsMention() + " (" + user.getAsTag() + ") from the server?").setEphemeral(true).addActionRow(
                    Button.success("bancmd_yes", Cameron.getExistingEmoji("greentick")),
                    Button.danger("bancmd_no", Cameron.getExistingEmoji("redtick"))
            ).queue();
        }
    }
    ...
    public void onButtonInteraction(@NotNull ButtonInteractionEvent event) {
        ...
        if(componentId.equals("bancmd_no")) {
            event.editMessage("Cancelled.").setActionRows().queue();
            return;
        }
        if(!componentId.equals("bancmd_yes")) return;
